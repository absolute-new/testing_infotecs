...
Файл `logger.conf` должен находиться в **текущей рабочей директории**, из которой ты запускаешь `logger`.  
Поэтому запускай `logger` из папки `build/logger/`, где он уже лежит автоматически.
